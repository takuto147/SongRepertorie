...
| サインイン画面。<br>"USER ID"にはメールアドレス、"ACCESS CODE"にはパスワードを入力。<br>("CREATE NEW ACCOUNT"以下は特に機能なし) | ホーム画面。登録済みの曲の一覧を表示し、タグ・キー・スコアなどを確認できる。<br>フィルター/ソート/検索機能で欲しい曲データのみを表示させることが可能。　|
...
| 検索画面。曲名やアーティスト名で検索することが可能。<br>"TRENDING SONGS"はそれっぽく表示しているだけで曲名は固定。 | 検索結果画面。<br>曲を選択するとその曲をもとにした登録画面が表示される。　|
